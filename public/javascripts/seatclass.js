...
if (path.match('/seat-classes/addseatclass')) {
    // validate add seat class form
    $("#add-seatclass-form").validate({
        rules: {
            "seatClassName": {
                required: true,
            },
        ...
        messages: {
            "seatClassName": {
                required: "Please enter a seat class name",
            },
        },
    ...
    // handle submit add seat class form
    function submitForm() {
        const seatClassName = document.getElementById("seatClassName").value;
        const data = {
            seatClassName: seatClassName,
        }
        $.ajax({
            contentType: "application/json",
            url: '/seat-classes/addseatclass/',
            dataType: "json",
            ...
            window.location.href = "/seat-classes";
        }).fail((res) => {
            $("#errorMessage").empty();
...
if (path.match('/seat-classes/updateseatclass/')) {
    $("#update-seatclass-form").validate({
        rules: {
            "seatClassName": {
                required: true,
            },
    ...
    function submitForm() {
        ...
        $.ajax({
            contentType: "application/json",
            url: '/seat-classes/updateseatclass/',
            dataType: "json",
            type: 'POST', // http method
            data: JSON.stringify(data), // data to submit
        }).done((res) => {
